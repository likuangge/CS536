...
public class P6 {
	 ...
	 * it 
	 * @param args command line args array for [<infile> <outfile> <spimfile>]
	 */
	private P6(String[] args){
        ...
        if (args.length < 3) {
        	String msg = "please supply name of file to be parsed"
					...
					+ " and the file for code generation";
        	pukeAndDie(msg);
        }
	 ...
	 * @param filename path to destination file
	 */
	public void setSpimfile(String filename) throws BadOutfileException{
        try {
            spimFile = new PrintWriter(filename);
        } catch (FileNotFoundException ex) {
        	throw new BadOutfileException(ex, filename);
        }
	}
	...
	public void cleanup(){
		...
		// Clean up spimFile as we do outFile //
		if (spimFile != null){
			spimFile.flush();
			spimFile.close();
		}
	...
	public int process(){
		...
		// Code Generation //
		Codegen.p = spimFile;//TODO if this doesn't work, may need a new one
		astRoot.codeGen(); // perform code generation
		Codegen.p.close();
