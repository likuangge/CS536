...
public class P4 {
        ...
        throws IOException // may be thrown by the scanner
    {
		...
		// Perform name analysis on AST //
		((ProgramNode)root.value).analyzeNames();

		// Unparse ONLY if we had no name analysis errors //
		if(!ErrMsg.isFatalError){
        	((ASTnode)root.value).unparse(outFile, 0);
			outFile.close();
		}
		else
			System.out.println("Errors occured during name analysis - skipping unparsing");
